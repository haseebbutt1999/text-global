...
//                                    $this->log_store->log_store( $shop->id, 'Orderdispatch', $lapsed_campaign->id,$messgae_text, $lapsed_campaign->campaign_name, 'Failed');
//                                    //
//                                    $test->save();
//                                } else {
//                                    $response = json_decode($response);
//                                    if($response->messages[0]->status->name == "PENDING_ENROUTE"){
//                                        $this->log_store->log_store($shop->id, 'Orderdispatch', $lapsed_campaign->id,$messgae_text, $lapsed_campaign->campaign_name, 'Sent');
//                                        $this->user_log->user_log( $shop->id, $customer->phone,$customer->first_name,$customer->last_name,$messgae_text,'Orderdispatch', $order_dispatch_data->name , $order_dispatch_data->customer->id, 'Sent', "sended");
//
//                                        //                Detect Credits
...
//                                        $test->save();
//                                        $this->user_log->user_log( $shop->id, $customer->phone,$customer->first_name,$customer->last_name,$messgae_text,'Orderdispatch', $order_dispatch_data->name , $order_dispatch_data->customer->id, 'Failed', "not sended");
//                                        $this->log_store->log_store($shop->id, 'Orderdispatch', $lapsed_campaign->id,$messgae_text, $lapsed_campaign->campaign_name, 'Failed');
//
//                                    }
//
