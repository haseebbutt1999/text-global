...
//                        $auth = "Basic ". base64_encode("$username:$password");
                    ...
                    if ($err) {
                        ...
                        if($response->messages[0]->status->name == "PENDING_ENROUTE"){
                            ...
                            $this->user_log->user_log( $shop->id, $customer->phone,$customer->first_name,$customer->last_name,'Welcomecampaign',null, $pushed_cust->shopify_customer_id, 'Welcome Sms Sended Successfully to new Customer ('.$customer->first_name.')', "sended");
                            //                Detect Credits
                            ...
                            $test->save();
                            $this->user_log->user_log( $shop->id, $customer->phone,$customer->first_name,$customer->last_name,'Welcomecampaign',null, $pushed_cust->shopify_customer_id, 'Welcome SMS not Sended to Customer ('.$customer->first_name.') because '.$response->messages[0]->status->description, "not sended");
                            $this->log_store->log_store($shop->id, 'Welcomecampaign', $welcome_campaign->id, $welcome_campaign->campaign_name, 'Welcome Sms not Sended');
                        }
                    }
                }
